...
### Example 1: Cascade Update Hook

Update related records when a parent record changes status.

```typescript
// packages/crm/src/opportunity_cascade.hook.ts
import type { Hook, HookContext } from '@objectstack/spec/data';

const OpportunityCascadeUpdate: Hook = {
  name: 'OpportunityCascadeUpdate',
  object: 'opportunity',
  events: ['afterUpdate'],
  handler: async (ctx: HookContext) => {
    ...
    if (opp.stage !== 'closed_won' || prev?.stage === 'closed_won') return;

    // Update the account's last_won_date
    await ctx.ql.update('account', opp.account_id, {
      last_won_date: new Date().toISOString(),
      total_won_amount: { $inc: opp.amount }
    });

    // Create a follow-up task for the sales rep
    await ctx.ql.insert('task', {
      subject: `Post-Sale Follow-up: ${opp.name}`,
...
export default [OpportunityCascadeUpdate];
```

### Example 2: Validation Hook with Cross-Object Checks

Prevent actions based on related record state.

```typescript
// packages/finance/src/payment_validation.hook.ts
import type { Hook, HookContext } from '@objectstack/spec/data';

const PaymentValidation: Hook = {
  name: 'PaymentValidation',
  object: 'payment',
  events: ['beforeInsert'],
  handler: async (ctx: HookContext) => {
    const payment = ctx.input.doc as Record<string, any>;

    // Fetch the related invoice
    const invoice = await ctx.ql.findOne('invoice', payment.invoice_id);

    if (!invoice) {
      throw new Error('Cannot create payment: Invoice not found.');
    }

    if (invoice.status === 'Paid') {
      throw new Error('Cannot create payment: Invoice is already fully paid.');
    }

    // Calculate remaining balance
    const existingPayments = await ctx.ql.find('payment', {
      filters: [['invoice_id', '=', payment.invoice_id]]
    });
...
### Example 3: Action with AI Integration

Define an action callable by AI agents and API endpoints.

```typescript
// packages/crm/src/deal_summary.action.ts
import type { ActionDefinition } from '@objectstack/spec';

export const generateDealSummary: ActionDefinition = {
  name: 'generate_deal_summary',
  label: 'Generate Deal Summary',
  object: 'opportunity',
  handler: async (params: { opportunityId: string }, ctx: any) => {
    const opp = await ctx.ql.findOne('opportunity', params.opportunityId);
    const activities = await ctx.ql.find('activity', {
      filters: [['related_to', '=', params.opportunityId]],
      sort: { created_at: -1 },
      limit: 10
    });
    ...
    const summary = {
      ...
      risk_factors: opp.amount > 100000 && activities.length < 3
        ? ['Low engagement for high-value deal']
        : []
    };
