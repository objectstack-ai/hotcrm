...
└── base.tsconfig.json     # Standard TypeScript config
```

## Package Overview

### Engine: `@objectstack/runtime`

**Purpose**: The core runtime that powers the entire system. It is installed as a dependency, not developed in this repo.

**Capabilities**:
- **ObjectQL Engine**: Handles database queries.
- **Metadata Registry**: Loads schemas from `packages/*`.
- **API Server**: Auto-generates REST/GraphQL endpoints.

### Domain Packages (Functions)

Each package in `packages/` is a plugin that contributes functionality to the runtime.
...
**Dependencies**: `@objectstack/runtime` (peerDependency)
