...
import * as salesObjects from '../objects';

/**
 * Activity-logging actions — `log_call`, `log_meeting`, `schedule_meeting`.
 *
 * (It was `src/actions/global.actions.ts` until the ADR-0130 layout: nothing
 * here is a *global* action, and nothing can be — see the dispatcher note on
 * `objectName` below. The name now says what it builds.)
 *
 * # Why the factory lives in the SALES package
 *
 * Four of the five host objects — `crm_lead`, `crm_contact`, `crm_account`,
 * `crm_opportunity` — are sales', as is the `crm_event` every action writes.
 * The fifth, `crm_case`, is service's, and under plan item 7 a source more than
 * one package needs takes the lowest package all its consumers depend on. So
 * the factory is here and `src/service/actions/case-activity.actions.ts`
 * instantiates the case triple from it along the service -> sales edge. One
 * copy, never two.
 *
 ...
 * on.
 *
 * The object definitions are passed IN rather than imported here, because the
 * five host objects no longer live in one directory: `crm_case` is service's.
 * A package hands {@link activityActionsFor} its own object barrel, so the map
 * is still derived from the real definition of the object being bound and no
 * package reaches into another's.
 */
type NameFieldSource = { name?: unknown; nameField?: unknown };

const nameFieldMap = (objectDefs: Record<string, NameFieldSource>): Record<string, string> =>
  Object.fromEntries(
 ...
 * body is written for.
 */
function activityAction(spec: ActivitySpec, objectName: string, nameField: string): Action {
...
/**
 * The three activity actions for ONE host object — the factory every package
 * with an activity target calls.
 *
 * Returned keyed by kind rather than as a list, so the named exports below read
 * as themselves and a caller cannot pick the wrong index. Each package calls
 * this ONCE per host object and exports the three it gets back, so a page that
 * imports `LogCallAction` and the barrel that registers it hold the SAME
 * object: two structurally-identical actions under one `<objectName>:<name>`
 * registry key is a duplicate registration the runtime warns about, and one of
 * them silently loses.
 *
 * `objectDefs` is the calling package's object barrel — see the note on
 * {@link nameFieldMap} for why the definitions arrive as an argument.
 */
export const activityActionsFor = (
  objectName: string,
  objectDefs: Record<string, NameFieldSource>,
): Record<(typeof ACTIVITY_SPEC_NAMES)[number], Action> => {
  // `?? 'name'` only fires for an object that declares no nameField at all,
  // which no business object in this app does.
  const nameField = nameFieldMap(objectDefs)[objectName] ?? 'name';
  const built = ACTIVITY_SPECS.map((spec) => activityAction(spec, objectName, nameField));
  return Object.fromEntries(built.map((a) => [a.name, a])) as Record<
    (typeof ACTIVITY_SPEC_NAMES)[number],
    Action
  >;
};

/*
 * The family, named. `src/sales/actions/index.ts` re-exports exactly these —
 * the stack is built from `Object.values(actions)`, so every export it forwards
 ...
 */
const leadActivityActions = activityActionsFor('crm_lead', salesObjects);
const contactActivityActions = activityActionsFor('crm_contact', salesObjects);
const accountActivityActions = activityActionsFor('crm_account', salesObjects);
const opportunityActivityActions = activityActionsFor('crm_opportunity', salesObjects);

export const LeadLogCallAction: Action = leadActivityActions.log_call;
export const LeadLogMeetingAction: Action = leadActivityActions.log_meeting;
export const LeadScheduleMeetingAction: Action = leadActivityActions.schedule_meeting;

export const ContactLogCallAction: Action = contactActivityActions.log_call;
export const ContactLogMeetingAction: Action = contactActivityActions.log_meeting;
export const ContactScheduleMeetingAction: Action = contactActivityActions.schedule_meeting;

export const AccountLogCallAction: Action = accountActivityActions.log_call;
export const AccountLogMeetingAction: Action = accountActivityActions.log_meeting;
export const AccountScheduleMeetingAction: Action = accountActivityActions.schedule_meeting;

export const OpportunityLogCallAction: Action = opportunityActivityActions.log_call;
export const OpportunityLogMeetingAction: Action = opportunityActivityActions.log_meeting;
export const OpportunityScheduleMeetingAction: Action = opportunityActivityActions.schedule_meeting;

/*
 * The `crm_case`-scoped triple is NOT built here. `crm_case` is the service
 * package's object, so service instantiates it from this factory — see
 * `src/service/actions/case-activity.actions.ts`, which keeps the original
 * `LogCallAction` / `LogMeetingAction` / `CaseScheduleMeetingAction` export
 * names because `src/service/pages/case_detail.page.ts` imports them by those
 * names.
 */
