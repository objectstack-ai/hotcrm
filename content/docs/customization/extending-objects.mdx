...
The simplest extension. To add a field to an existing object (e.g., `opportunity`), create a *field augmentation* in your own package.
...
| Use case | Field type |
| --- | --- |
| Parent reference (optional) | `Field.lookup({ reference_to: 'account' })` |
| Required parent-child | `Field.masterDetail({ reference_to: 'opportunity' })` |
| Rollup (count/sum/min/max) | `Field.summary({ reference: 'opportunity_line_item', operation: 'sum', field: 'total_price' })` |
...
export default defineHook({
  ...
  handler: async ({ record, broker, context }) => {
    ...
    if (record.account) {
      const acc = await broker.findOne('account', { id: record.account });
