...
- **日期範圍** — CRM Overview、Sales Performance 和 Executive Overview 各帶一個以商機預計成交日期為準的選擇器，預設本季。Sales Activity 和 Customer Service 沒有，原因見上面各自的章節。每一個時間窗都由伺服器端解析，按 UTC 日曆日算："最近 7 天"指的是以今天的 UTC 日期結尾的那七個 UTC 日曆日，兩端都含。你自己的時鐘不參與其中——一位在 UTC+8 的讀者在當地時間凌晨 3 點打開儀表板，看到的時間窗是在上一個 UTC 午夜結束的，而不是在他自己的午夜。
