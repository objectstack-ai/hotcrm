...
每个工单相关的仪表盘磁贴与全部三份工单报表背后的那个 dataset。六个维度 —— **Status**、**Priority**、**Origin**、**Type**、**Created**（按天分桶）、**Resolving Article** —— 与八个度量：

- **Cases** —— 工单数（`case_count`）。
- **Avg Resolution (h)** —— 平均解决时长（`avg_resolution`）。
- **SLA 违约率** —— **SLA Violated** 标志的平均值（`avg_sla_violated`），它是违约率，不是达标率。
- **Closed Cases** —— 已关闭工单数（`closed_count`），下面两个比率共用的分母。
- **Resolved by KB** —— 带有解决知识文章的已关闭工单数（`kb_resolved_count`）。
- **KB Deflection Rate** —— **Resolved by KB** ÷ **Closed Cases**（`kb_deflection_rate`），声明为派生比率，任何小部件都不必临时凑一个分母。
- **Cases Within SLA** —— 未违约的已关闭工单数（`sla_met_count`）。
- **SLA Compliance Rate** —— **Cases Within SLA** ÷ **Closed Cases**（`sla_compliance_rate`）；这就是达标率，**SLA 违约率**的补数。

**这里没有首次响应度量。** `case_metrics`（`src/datasets/case.dataset.ts`）没有声明任何一个，所以分析侧根本无法对首次响应做聚合、比较或出图。但「SLA 达标 %」*是*存在的 —— 就是上面的 **SLA Compliance Rate**，服务仪表盘的 SLA 仪表盘图正是在画它。工单上的 **First Response Date** 时间戳是真的 —— 它何时被写入见 [工单](/zh-Hans/docs/service/cases) —— 但也就到此为止：没有任何东西度量它，也没有任何东西为它告警。

本节曾经列出的其余条目同样够不着：

- **未结 / 新建 / 已解决工单数** —— 没有任何度量是按*这三个*状态预筛的。**Status** 是维度，所以按状态*拆开*的计数能给出同样的分解。这个 dataset 确实声明了预筛计数，但只在已关闭这一侧：**Closed Cases**、**Resolved by KB** 与 **Cases Within SLA**。
