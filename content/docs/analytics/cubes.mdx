...
| **Case Metrics** (`case_metrics`) | `crm_case` | Status, Priority, Origin, Type, Created (day), Resolving Article | Cases, Avg Resolution (h), SLA Violation Rate, Closed Cases, Resolved by KB, KB Deflection Rate, Cases Within SLA, SLA Compliance Rate |
...
The dataset behind every case-related dashboard tile and all three case reports. Six dimensions — **Status**, **Priority**, **Origin**, **Type**, **Created** (bucketed by day), **Resolving Article** — and eight measures:

- **Cases** — the case count (`case_count`).
- **Avg Resolution (h)** — average resolution time (`avg_resolution`).
- **SLA Violation Rate** — the average of the **SLA Violated** flag (`avg_sla_violated`), which is a breach rate and not a compliance percentage.
- **Closed Cases** — the count of closed cases (`closed_count`), the denominator both rates below divide by.
- **Resolved by KB** — closed cases that carry a resolving article (`kb_resolved_count`).
- **KB Deflection Rate** — **Resolved by KB** ÷ **Closed Cases** (`kb_deflection_rate`), declared as a derived ratio so no widget improvises a denominator.
- **Cases Within SLA** — closed cases that did not breach (`sla_met_count`).
- **SLA Compliance Rate** — **Cases Within SLA** ÷ **Closed Cases** (`sla_compliance_rate`); this is the on-time percentage, the complement of **SLA Violation Rate**.

**There is no first-response measure here.** `case_metrics` (`src/datasets/case.dataset.ts`) declares none, so nothing in analytics can aggregate, compare or chart a first response. An "SLA met %" *does* exist — **SLA Compliance Rate** above — and the Service dashboard's SLA gauge plots it. The **First Response Date** stamp on the case is real — see [Cases](/docs/service/cases) for when it is written — but it stops there: nothing measures it, and nothing alerts on it.

The rest of what this section used to list is not reachable either:

- **Open / New / Resolved case count** — no measure is pre-filtered to *those* three states. **Status** is a dimension, so a count *split by* status gives the same breakdown. The dataset does declare pre-filtered counts, but on the closed side only: **Closed Cases**, **Resolved by KB** and **Cases Within SLA**.
