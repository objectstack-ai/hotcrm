...
本节曾经列出的另外五份报表哪儿都没有发布，而且其中多数是在向语义层要它根本没有的东西。`case_metrics`（`src/datasets/case.dataset.ts`）只读 `crm_case` 一个对象，六个维度 —— **Status**、**Priority**、**Origin**、**Type**、**Created**（按天分桶）、**Resolving Article** —— 外加八个度量：**Cases**、**Avg Resolution (h)**、**SLA Violation Rate**、**Closed Cases**、**Resolved by KB**、**KB Deflection Rate**、**Cases Within SLA** 与 **SLA Compliance Rate**。
