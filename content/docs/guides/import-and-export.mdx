---
title: Import & Export
description: What actually ships for getting data in and out — the list-view import wizard and HotCRM's three ready-made mappings, CSV / XLSX export from four list views, the data API — and which migration wizards are still design intent.
---
...
Four list views declare `exportOptions: ['csv', 'xlsx']` — **Accounts**, **Contacts**, **Leads** and **Opportunities**. Open one, use **Export** in the toolbar, pick CSV or Excel. What you get is the view you are looking at: its filter, its sort and its columns travel with the export.
...
Cases carry an export grant but no Cases list view declares `exportOptions`, so there is no **Export** button on the Cases list. The grant is still live: export runs on one server-side route, the same one the Export button calls, so a role holding it exports Cases over the data API below. Reports are not an export surface here — a report page renders its chart and table, and offers no download.
