...
Nothing counts down in this app — there is no live SLA timer to watch. What exists is a due date stamped on Critical cases and an hourly sweep that reports a breach after the fact. Check:
...
- The case has an **SLA Due Date** at all. Only **Critical** gets one automatically: `src/objects/case.hook.ts` stamps `sla_due_date` four hours out, and no other priority is stamped. A case with a blank due date is never a candidate for the sweep, so **SLA Violated** stays `false` however long it stays open. (Typing a due date in by hand works — the sweep then treats that case like any other.)
...
**There is no business-hours setting to check** — no working-day calendar, no holiday list, no *Setup → Business Hours* screen anywhere in this app. Those four hours are wall-clock hours, and nights, weekends and holidays count against them. **There is also no per-priority SLA definition and no default SLA**: the four-hour Critical target is the only one the app keeps, written into the hook rather than configured, and High / Medium / Low are service promises your team keeps by hand. See [SLA & Escalation](/docs/service/sla-and-escalation).
