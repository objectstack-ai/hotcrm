...
| **Source** | `scheduled` (nightly job), `ai` (reserved for an agent or integration of your own — nothing in HotCRM writes it), `manual` (rep entry). Not only a label: on the current quarter, an `ai` or `manual` row **stands the nightly job down** for that owner and period — see *How forecasts get created* below. |
