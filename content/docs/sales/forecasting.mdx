...
**What you do supply is kept exactly as you sent it.** The derivation fills in the blanks; it never rewrites a value that arrived with the record. Send `period_start` 2026-07-15 for a quarterly snapshot and the row is stored with that date — labelled **Q3 2026**, but starting mid-quarter rather than on the quarter's first day. Nothing snaps it back, and the surfaces that scope by period will then not find it: the forecast list's *This Quarter* tab and the Sales dashboard's *Quota Attainment by Rep* table both match `period_start` against the quarter's real first day. So when you write a snapshot by hand or through the API, send the period's first day — or send no `period_start` at all and let the derivation place it.
...
- **System administrators** reach every forecast as well: the System Administrator profile holds the same *View All* / *Modify All*, plus delete. Those two profiles are the whole list — HotCRM ships [six profiles](/docs/administration/profiles) and none of them is a *Sales Operations* one, so whoever calibrates other people's numbers has to hold **Sales Manager** or **System Administrator**.
