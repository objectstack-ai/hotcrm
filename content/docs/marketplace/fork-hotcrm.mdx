...
- File-suffix protocol (`*.object.ts`, `*.hook.ts`, `*.action.ts`)
- ObjectQL (no raw SQL)
- AI tool exposure via `*.action.ts`
- Strict `@objectstack/spec` schema validation
- i18n bundles for all locales you ship
- Profile + role hierarchy + sharing rules
- Approval processes
- Hooks correctly lowered for the build pipeline
- Dashboards backed by analytics cubes
...
```bash
# macOS (BSD sed)
find src apps/docs/content -type f \( -name "*.ts" -o -name "*.mdx" -o -name "*.json" \) \
...
| API + AI tool | `src/actions/<entity>.action.ts` |
...
pnpm typecheck
pnpm build
pnpm dev    # smoke-test in the browser at http://localhost:3000
```
