...
/**
 * Sales seeds — accounts, contacts, leads and opportunities.
 *
 * Split out of the former monolithic `src/data/index.ts` (#635). Seed doctrine
 * lives in `./_shared.ts`.
 *
 * `OPPORTUNITY_LINES` stays here, with the deals it prices: `dealValue()`
 * derives every opportunity's `amount` from it. The line-item ROWS it also
 * feeds are `crm_opportunity_line_item`, a revenue object, so they are seeded
 * from `src/revenue/data/opportunity-line-item.seed.ts`, which reads this
 * table along the revenue -> sales edge.
 */
