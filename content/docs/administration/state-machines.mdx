...
**Allowed transitions** — this is the whole table, copied from the `lead_status_progression` rule in `src/sales/objects/lead.object.ts`, and it carries a few shortcuts the picture above leaves out:
...
**Allowed transitions**, copied from the `opportunity_stage_progression` rule in `src/sales/objects/opportunity.object.ts`:
...
If you need a status change to be genuinely impossible, the state machine is the wrong tool: an `error`-severity validation rule is the one that blocks a save. Keeping these five advisory is deliberate — the reasoning is written into the rules themselves (`src/revenue/objects/quote.object.ts`): a support-driven correction must not require a data fix.
