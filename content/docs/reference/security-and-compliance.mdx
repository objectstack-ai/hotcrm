...
> `package.json` declares — on **17.1.0**, the version installed at the time of that
> measurement. ⚠️ **That is not the version installed now:** `package.json` has pinned
> **17.2.0** since 2026-09-01, and this page has not been re-checked against it. The
> claims below are deliberately **not** re-dated to the newer pin — re-dating a
> measurement nobody re-took would replace a stale warrant with a fabricated one, which
> is the worse of the two errors on a page like this. Headings and rows
