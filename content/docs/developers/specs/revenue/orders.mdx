...
3.  **Subscriptions**: If `product.type` == 'Subscription', creates `subscription` record.
4.  **Assets**: If `product.type` == 'Asset', creates `asset` record for the Account.

## Invoice Run

The `InvoiceScheduler` runs nightly.
1.  **Select**: Findings `order` records with `next_billing_date` <= Today.
