...
const logger = createLogger('crm:cross_cloud_lifecycle');
...
export const opportunityToOrderLifecycle = async (params: { opportunity_id: string }) => {
  logger.info('Cross-cloud: Opportunity → Order → Invoice lifecycle', params);
  ...
  logger.info('Lifecycle complete: Opportunity →', (order as any)._id, '→', (invoice as any)._id);
...
export const campaignAttributionChain = async (params: { campaign_id: string; lead_id: string }) => {
  logger.info('Cross-cloud: Campaign → Lead → Opportunity attribution', params);
  ...
  logger.info('Attribution chain: Campaign', campaign_id, '→ Lead', lead_id, '→', opps.length, 'opportunities');
...
export const forecastRevenueAlignment = async (params: { forecast_id: string }) => {
  logger.info('Cross-cloud: Forecast → Revenue Recognition alignment', params);
  ...
  logger.info('Alignment: Forecast', forecast.amount, 'vs Recognized', totalRecognized);
...
export const caseKnowledgeFeedback = async (params: { case_id: string }) => {
  logger.info('Cross-cloud: Case → Knowledge Article feedback loop', params);
  ...
  logger.info('Knowledge feedback: Case', case_id, '→', suggestions.length, 'article suggestions');
