...
describe('Invoice Prediction - predictPaymentDefault', () => {
  ...
  it('should predict default probability', async () => {
    ...
    (broker.findOne as Mock).mockResolvedValueOnce(mockInvoice).mockResolvedValueOnce(mockAccount);
  ...
  it('should analyze payment history', async () => {
    ...
    (broker.findOne as Mock).mockResolvedValueOnce(mockInvoice).mockResolvedValueOnce(mockAccount);
    (broker.find as Mock).mockResolvedValue(historicalInvoices);
  ...
  it('should flag high-risk invoices', async () => {
    ...
    (broker.findOne as Mock).mockResolvedValueOnce(mockInvoice).mockResolvedValueOnce(mockAccount);
    (broker.find as Mock).mockResolvedValue(badHistory);
...
describe('Invoice Prediction - predictPaymentDate', () => {
  ...
  it('should predict payment date', async () => {
    ...
    (broker.findOne as Mock).mockResolvedValue(mockInvoice);
    (broker.find as Mock).mockResolvedValue([]);
...
describe('Invoice Prediction - detectAnomalies', () => {
  ...
  it('should detect invoice anomalies', async () => {
    ...
    (broker.findOne as Mock).mockResolvedValue(mockInvoice);
    (broker.find as Mock).mockResolvedValueOnce(mockLines).mockResolvedValueOnce([{ amount: 50000 }]);
  ...
  it('should detect calculation errors', async () => {
    ...
    (broker.findOne as Mock).mockResolvedValue(mockInvoice);
    (broker.find as Mock).mockResolvedValueOnce(badLines).mockResolvedValueOnce([]);
