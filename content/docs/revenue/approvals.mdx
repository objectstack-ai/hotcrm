...
### ↩️ Recalled (withdrawn by the submitter)
- The submitter can withdraw their own pending request from the approval request — nobody has decided anything, so this is not a rejection.
- `approval_status` is stamped **recalled** (shown as *Recalled*) and the record is unlocked. **No** "deal rejected" notification is sent: the person who would receive it is the person who just withdrew it.
- The withdrawal is captured in the approval request's history alongside decisions.
- A recalled deal is **not** resubmitted automatically. Approval re-launches on save only from *Not Required*, so a recall — like a rejection — parks the deal until an admin resets the status.
