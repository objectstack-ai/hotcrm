...
**HotCRM 3.0.0**, running on **ObjectStack 17.2.0** across every
`@objectstack/*` package. The app manifest declares the matching
`^17.2.0` protocol range, so a runtime on a different protocol major
...
> entry every pull request adds. The versions between v1.0 and 3.0.0 are
