...
Cases carry an export grant but no Cases list view declares `exportOptions`: that grant is aimed at report export, which rides on the platform's `reports` capability. This app's `requires` does not list it, so on a default boot the four list views above and the data API are the export surfaces you have.
