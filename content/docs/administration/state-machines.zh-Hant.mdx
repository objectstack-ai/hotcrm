...
- **工單** —— *New → In Progress → Resolved → Closed*；*Waiting on Customer* 與 *Waiting on Support* 用來把未結案的工單掛起，*Escalated* 從任何未解決的狀態都能進入，*Resolved* 與 *Closed* 都可以重新開啟回 *In Progress*
...
五者的宣告方式完全相同 —— 都是物件上一條具名的 `state_machine` 驗證規則，作用於該物件的生命週期欄位（`status`，商機上是 `stage`）。工單、合約、報價用的不是比潛在客戶和商機更輕的機制，就是同一套機制。頂層 `stateMachines` 鍵在平台 7.7 被移除後，所有狀態機都遷到了各自的物件上。
...
**允許的轉換** —— 下面就是整張表，照抄自 `src/sales/objects/lead.object.ts` 裡的 `lead_status_progression` 規則；它比上面那張圖多帶幾條捷徑：
...
**允許的轉換**，照抄自 `src/sales/objects/opportunity.object.ts` 裡的 `opportunity_stage_progression` 規則：
...
- **審批的入口是 $100K。** `opportunity_approval` 流程會在任何一次更新之後觸發，只要這筆開放商機金額達到 **$100,000 或以上** 且還沒有在案的審批，就把它送到 **銷售經理** 那裡。**$500K** 是第二檔而不是入口：超過這個數，還要 **銷售總監** 在經理之後加簽。所以一筆 $200K 的單子是會進審批的。
...
如果你需要某個狀態變更真正不可能發生，狀態機不是那個工具：能擋下儲存的是 `error` 級的驗證規則。這五條保持建議級是有意的 —— 理由就寫在規則自己身邊（`src/revenue/objects/quote.object.ts`）：售後的更正不該還要靠一次資料修復才能做到。
...
上面這些都是改原始碼，不是在 Setup 裡點幾下：**並不存在「Setup → Object → Status → State Machine」這個頁面**。每張轉換表都是物件上的一條 `validations[]` 條目（型別 `state_machine`，見 `src/*/objects/*.object.ts`），而這條條目只帶一張轉換對應表，別的什麼都沒有 —— 所以上面前兩項是改這張對應表，進入/退出條件是另外的 `validations[]` 規則或相關欄位上的 `requiredWhen` 述詞，按轉換觸發自動化則是 `record_change` 流程或物件鉤子。每一項都是程式碼改動加一次重新部署。
...
- ✅ **不要把自動化掛在轉換上** —— 根本沒有可掛的東西。轉換表是一條驗證規則，在做出這次移動的那一次儲存裡被求值；它寫下一行日誌，不產生任何可供自動化訂閱的事件。只應在某一次移動上跑的自動化，是一條 `record_change` 流程：它在那次儲存上觸發，並在自己的起始條件裡把 `record.stage` 與 `previous.stage` 比對來收窄 **自己** —— `opportunity_won_alert`（`src/sales/flows/opportunity-won-alert.flow.ts`）正是這麼寫的，所以那條祝賀通知只在移入 *Closed Won* 的那一次發出，而不是在這筆已贏商機之後的每一次編輯上都發。
