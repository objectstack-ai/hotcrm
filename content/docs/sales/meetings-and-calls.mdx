...
**Scheduling** a meeting produces the first two, not the third. The activity timeline is a record of what *happened*, and a meeting on next week's calendar has not happened yet. A booking is therefore found on the **calendar** and in the record's **Events** related list — both of which read the event itself, so nothing about it is hidden.
...
| Field | Notes |
| --- | --- |
| **Attendee Type** | *Contact*, *Lead* or *User* — says which kind of person the row names |
| **Contact / Lead / User** | The person, as a real link to their record |
| **External Attendee** | Free text, and the last resort: for a guest who is in no CRM object at all (the prospect's lawyer). Not a place to paste a list. |
...
A row must name a contact, a lead, a user, or an external guest — an attendee pointing at nobody is rejected.
...
In the **Activity** group of the sidebar:

| Entry | What it shows |
| --- | --- |
| **Events** | Every event you can see, newest first |
| **Calendar** | The same rows on a calendar, coloured by type |
| **Interaction History** | Held events only — what actually happened |

Under **My Work → My Calendar** you get your own events, and the Events list itself carries tabs for **All**, **Calendar**, **Team Schedule** (one lane per rep), **My Calendar**, **Upcoming** (planned, soonest first) and **Interaction History**.
