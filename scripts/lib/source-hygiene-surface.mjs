 ...
 *
 * ⛔ `pnpm-lock.yaml` is deliberately absent. It is text, and it is the largest
 * file at root, but size is not the argument: it is generated by pnpm, nobody
 * hand-edits it, and the hazard this check guards is a writing-time one (during
 * PR #835 an editing tool materialised escapes into real 0x01/0x00 bytes in a
 * file being authored, confirmed with `od -c`). The remedy this check prints —
 * "write the character as an escape sequence" — is advice to an author, and a
 * lock file has none; a hit there would come from a package manager and could
 * not be fixed in this repo. Admitting it later is one line, and should be its
 * own decision rather than a side effect of this one.
 *
 * This paragraph used to read "`pnpm-lock.yaml` and `package-lock.json`", and
 * pointed at `scripts/check-stackblitz-lock.mjs` as the pair's dedicated gate.
 * The second lockfile existed only for the StackBlitz demo; the demo, the
 * lockfile and that gate are all retired now. The byte figures went with them
 * — they had already drifted (221KB/320KB written against 260KB/397KB
 * measured), which is why the argument runs without them.
 */
