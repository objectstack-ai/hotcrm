...
**允许的转换** —— 下面就是整张表，照抄自 `src/sales/objects/lead.object.ts` 里的 `lead_status_progression` 规则；它比上面那张图多带几条捷径：
...
**允许的转换**，照抄自 `src/sales/objects/opportunity.object.ts` 里的 `opportunity_stage_progression` 规则：
...
如果你需要某个状态变更真正不可能发生，状态机不是那个工具：能挡下保存的是 `error` 级的验证规则。这五条保持建议级是有意的 —— 理由就写在规则自己身边（`src/revenue/objects/quote.object.ts`）：售后的更正不该还要靠一次数据修复才能做到。
...
- ✅ **不要把自动化挂在转换上** —— 根本没有可挂的东西。转换表是一条验证规则，在做出这次移动的那一次保存里被求值；它写下一行日志，不产生任何可供自动化订阅的事件。只应在某一次移动上跑的自动化，是一条 `record_change` 流程：它在那次保存上触发，并在自己的起始条件里把 `record.stage` 与 `previous.stage` 比对来收窄 **自己** —— `opportunity_won_alert`（`src/sales/flows/opportunity-won-alert.flow.ts`）正是这么写的，所以那条祝贺通知只在移入 *Closed Won* 的那一次发出，而不是在这笔已赢商机之后的每一次编辑上都发。
