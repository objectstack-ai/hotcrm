...
本節曾經列出的另外五份報表哪兒都沒有發布，而且其中多數是在向語義層要它根本沒有的東西。`case_metrics`（`src/datasets/case.dataset.ts`）只讀 `crm_case` 一個物件，六個維度 —— **Status**、**Priority**、**Origin**、**Type**、**Created**（按天分桶）、**Resolving Article** —— 外加八個度量：**Cases**、**Avg Resolution (h)**、**SLA Violation Rate**、**Closed Cases**、**Resolved by KB**、**KB Deflection Rate**、**Cases Within SLA** 與 **SLA Compliance Rate**。
