...
- *Lead Source ROI* does not exist either, and the semantic layer could not answer it as written: `lead_metrics` (`src/datasets/lead.dataset.ts`) declares a single measure — the lead count — and never crosses over to opportunities. Volume by source comes straight off it, because **Source** is one of its dimensions, and a conversion rate is arithmetic you do on top of a count grouped by status. The revenue half has nowhere to come from.
