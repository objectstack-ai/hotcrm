...
- There are no saved member list views — not *All Members*, *Responded*, *Bounced / Unsubscribed* or *Converted to Opportunity*. Campaign Member ships no list views at all, and a related-list panel would not read them if it did: the panel's columns are curated on the campaign link itself, never from a view. Nothing ships that narrows the panel to one status, and no report ships over Campaign Member either, so a subset — every bounce, every conversion — is a number on the campaign rather than a list: see [How the metrics roll up](#how-the-metrics-roll-up), where every rate is computed live from member status.
