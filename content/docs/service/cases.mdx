...
`crm_case` ships **eight** views (`src/service/views/case.view.ts`), and the case list surfaces them as tabs, each labelled with the view's own name:
...
- The SLA targets per priority and account tier are stored in the case object's automation. To change them, edit `src/service/objects/_case-sla.ts` and the matching table inside `case_sla_defaults` in `src/service/objects/case.hook.ts` — both, because a sandboxed hook body cannot import a constant — see [SLA & Escalation](/docs/service/sla-and-escalation) and [Administration › Automation](/docs/administration/automation).
