...
There is **no manual override**. Probability is a read-only field: every save re-derives it from the stage, so a number typed against the table above never reaches the record. Move the deal to the stage that reflects reality and the forecast follows — which is also what stops a rep inflating a forecast by hand.
