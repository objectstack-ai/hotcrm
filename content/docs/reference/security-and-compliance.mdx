...
> whole installed `@objectstack/*` tree — every package installed, not only the ones
> `package.json` declares — and **re-measured against 17.2.0 on 2026-09-03**, the version
> pinned on that date. Every claim those two sources can answer was re-taken rather than
> carried forward, and three did not survive the re-check: they are corrected below, not
> re-dated around. That re-measurement has not been repeated since, and the pin has moved
> on from 17.2.0 — so read what follows as a measurement dated to that version, not as a
> description of the release you run today. What a re-measurement here cannot reach is the
> half of each *(your deployment)* row that belongs to your instance and to whoever
> operates it; those keep the scope their marker gives them. Headings and rows
...
| Account lockout threshold | `lockout_threshold` | **0 — password-stage lockout disabled** |
...
- **"Account lockout after N failed attempts (configurable; default 5)."** The default is **`0`, which disables the password-stage lockout**. Five was never the default. Lockout is enforced per identity (so it survives IP rotation, unlike the rate limit beside it), and it is worth turning on — but a security review that assumed it was already on would have been reading the opposite of the truth. ⚠️ But `0` does not mean "no limit anywhere", and it does not mean the account is protected either. Two-factor verification keeps **a built-in limit of 10 attempts per 15 minutes** that no setting reaches, because it is the last check before a session is issued. That limit guards the *second* factor only, and two-factor is off by default here (see MFA below) — so a stock deployment signs in with a password alone and has no account lockout at all.
...
| **Login hours** | *(not shipped)* | Zero hits for any login-hours, login-window or allowed-hours surface across the 1,204 non-map distribution files of the 51 installed packages |
...
- A `dataResidency` shape (`region`, `excludeRegions`) does exist, in `@objectstack/spec/system`'s `TenantSecurityPolicySchema`, alongside a `compliance` block naming `sox | hipaa | gdpr | pci_dss | iso_27001 | fedramp`. It is exported and typed, so an author *can* write it — and **nothing anywhere outside `@objectstack/spec` reads it**, which is the half that decides the question. It describes an intention, not a control.
...
- **Suppression list** — ✅ verified. Both enrollment gates in `src/marketing/flows/campaign-enrollment.flow.ts` filter on `email_opt_out: false`, so an opted-out person is excluded from future campaign enrollment. The flag lives on the person, which is what makes the exclusion global rather than per campaign.
...
- ✅ **Set `lockout_threshold`.** It is `0` today, which leaves the password stage with no lockout at all — and on a stock deployment that is the only stage there is. This is the single most consequential default on the page.
