...
**Built-in flows in HotCRM** (26). Each row carries the flow's own label — the name you see in **Setup → Process Monitor** and **Setup → Scheduled Jobs**, so a run you are chasing in the monitor can be looked up here verbatim:
...
| **Large Deal Won Alert** | Record change (update) | When an opportunity over $100K turns *Closed Won*, notify the owner — the owner alone, not their manager |
...
| **Case Escalation Process** | Record change (update) | When a case turns *Critical*, flag it escalated and notify its owner — the case is not reassigned to a senior agent, it stays with its owner; escalating also opens an urgent follow-up task for the account owner |
