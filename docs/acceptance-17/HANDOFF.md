...
| 计划节 | 状态 |
|---|---|
| 0 自动化基线 | ✅ 完成(verify 1280 例、e2e 16/16、冷启动 0 ERROR) |
| 1 对象列表/详情 | ✅ 完成(15/17 通过,2 项因无种子数据阻塞) |
| 2 对象 CRUD | ✅ 完成(REST 17/17 全量通过) |
| 4 仪表盘/报表 | ✅ 完成(5 仪表盘 56 widget、10 报表全部出数) |
| 7 i18n 四语言 | ✅ 完成 |
| **3 actions 全量(26 个)** | ❌ **未执行** → 用 **P2** 包 |
| **5 flows 全量(24 条)** | ❌ **未执行** → 用 **P3** 包 |
| **6 profile 权限矩阵** | ❌ **未执行** → 用 **P4** 包 |
| **8 阻塞 issue 复测** | ⚠ **部分**:#520/#523 已确证解除;#521 #522 #509 #508 #524 #510 未测 → **P2**;#526 #528 未测 → **P5** |
