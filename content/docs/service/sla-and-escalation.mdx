...
**Every case gets an SLA due date automatically**, from a **priority × account-tier matrix**. The `case_sla_defaults` hook (`src/service/objects/case.hook.ts`) reads the case's **Priority** and the **Customer Tier** of its account, looks the pair up in the table below, and fills `sla_due_date` with *now + that many hours* — on create, or on the first edit that gives the case a priority, whenever the field is still empty.
...
The **SLA Performance** report breaks its numbers down by **one** dimension: **priority**. Its `rows` are `['priority']` and nothing else (`src/service/reports/case.report.ts`). What it reports is not a compliance percentage either, but the **SLA Violation Rate** — the average of the **SLA Violated** flag — shown next to the case count and the average resolution time, and computed over **closed cases only** (`runtimeFilter: { is_closed: true }`). (The on-time percentage itself is declared: **SLA Compliance Rate**, which the Service Overview dashboard's **SLA Compliance** gauge plots. This report simply does not carry it.) Of the four questions this page used to promise, it answers one: *which priority bucket is leaking?*

The other three are not merely missing from the report — they are unreachable in the semantic layer underneath it. The `case_metrics` dataset (`src/service/datasets/case.dataset.ts`) declares six dimensions — **Status**, **Priority**, **Origin**, **Type**, **Created** and **Resolving Article** — so:
...
A breach on a case **nobody owns** is handled the same way, and it is worth stating separately because it used to be the one case nobody was told about. The sweep escalates it like any other, the hand-off below gives it a Service Manager, and the alert goes to that new owner. If nobody holds the Service Manager position, the case stays unowned and no alert is sent — the breach is still recorded on the case and still shows in **Escalated Cases**, and the rest of the sweep runs as normal.

A breach on a case that **already has an owner** is handed over the same way, and its alert is addressed the same way the escalation's is: **the notification goes to the previous owner alone.** `case_sla_monitor` reads the recipient before the hand-off, so the breach alert reaches the agent the case is being taken from, not the service manager who now owns it. That is deliberate — one escalation etiquette on both paths — and it is the whole notice: the hand-off itself sends nothing of its own, so the manager who picks the case up finds it in **Escalated Cases** rather than in an inbox message.
