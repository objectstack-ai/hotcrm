...
export const Account = ObjectSchema.create({
  ...
  fields: {
     ...
     *
     * ## ⚠️ THIS FIELD GATES NOTHING YET, and that is not an oversight
     *
     * REQ-0003 is explicit that "a category no rule reads is prose, and the
     * rule is the point", and `objectstack validate` says the same thing back
     * about this declaration in as many words: "declared but nothing in this
     * stack reads or displays it … Verdict: inert".
     *
     * The rule REQ-0003 specifies for it — "Authored as a `validations[]` entry
     * on `crm_opportunity` reading its account's category" — CANNOT BE WRITTEN,
     * measured on the pinned packages rather than assumed:
     *
     *   • a `validations[]` predicate is evaluated by
     *     `checkPredicate(rule, merged, previous)`, which calls
     *     `ExpressionEngine.evaluate(expr, { record, previous })` — plain data
     *     objects, the opportunity's OWN stored columns and nothing else;
     *   • `record.crm_account.type` does not traverse the lookup. Measured:
     *     `runtime: No such key: type`;
     *   • `os.lookup("crm_account", …)` is not reachable either. `EvalContext`
     *     declares an optional `api.lookup`, but `buildScope()` in
     *     `@objectstack/formula` builds `os` from ONLY `ctx.user` / `ctx.org` /
     *     `ctx.env` and never binds `ctx.api` — so the call has no overload even
     *     when a caller passes one. Measured: `runtime: found no matching
     *     overload for 'dyn.lookup(string, dyn)'`. `checkPredicate` does not
     *     pass `api` at all;
     *   • and an unevaluable predicate REJECTS THE WRITE (#4649), so authoring
     *     it literally would refuse every opportunity write — insert and update
     *     alike — which is the bricking REQ-0003 acceptance 2 forbids, in its
     *     most extreme form.
     *
     * ⇒ The gate needs a construct decision that is not this file's to take
     * (hotcrm#1915). ⛔ Do NOT "fix" this by widening the predicate or by
     * denormalising the category onto `crm_opportunity` on the quiet: a copy
     * needs a writer and goes stale the moment an account is reclassified, and
     * a hook that can read the account can simply refuse, which is what
     * `lead.hook.ts` already does for the conversion gate.
     *
     * ⚠️ `events: ['insert']` on a validation rule IS honoured — measured in
     * `@objectstack/objectql`, which filters rules by `r.events ?? ['insert',
     * 'update']` against the write mode — so the transition-gate/invariant
     * distinction (AGENTS.md metadata semantics rule 7) is expressible here and
     * is NOT what blocks this. Only the cross-object read is.
     */
