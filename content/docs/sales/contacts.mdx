...
The contact detail screen has 7 collapsible sections:
...
| **Additional Info** | Lead source, description, last contacted |
| **Communication Preferences** | Primary contact flag, do-not-call, email opt-out |
...
- The list of departments (Executive, Sales, Marketing, Engineering, Support, Finance, Human Resources, Operations) is configurable per the picklist values. See [Customization › Extending Objects](/docs/customization/extending-objects).
- The new-contact prompt is a flow, not a template: edit `src/sales/flows/contact-welcome.flow.ts` to change its wording, its channels, or who it goes to. There is no email-template setting behind it.
