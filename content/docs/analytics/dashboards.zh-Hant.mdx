...
- **My Open Cases by Priority** — 目前登入使用者自己名下、每個優先順序的未結工單數及其 SLA 違約率（表格）。它不受 Agent 篩選影響，一律顯示你自己的工單。
...
- **日期範圍** — 五個儀表板裡有四個帶選擇器，且四個都支援自訂區間。CRM Overview、Sales Performance 和 Executive Overview 以商機預計成交日期取窗，預設本季；Customer Service 以工單創建日期取窗，預設最近 90 天。Sales Activity 是唯一沒有選擇器的，原因見上面它自己的章節。每一個時間窗都由伺服器端解析，按 UTC 日曆日算："最近 7 天"指的是以今天的 UTC 日期結尾的那七個 UTC 日曆日，兩端都含。你自己的時鐘不參與其中——一位在 UTC+8 的讀者在當地時間凌晨 3 點打開儀表板，看到的時間窗是在上一個 UTC 午夜結束的，而不是在他自己的午夜。
