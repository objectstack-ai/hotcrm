...
> **Note on `ObjectSchema.create()` vs `ObjectSchema.parse()`**: v3.0.0 introduced `ObjectSchema.create()` as the recommended lightweight API for defining objects. It provides the same type safety with less overhead than `parse()`. All 69 HotCRM objects use `ObjectSchema.create()`.
...
|-----------|--------|--------------|
| `ObjectSchema.create()` | ✅ **Used** | All 69 objects defined via `ObjectSchema.create()` — the recommended v3.0.0 lightweight API |
