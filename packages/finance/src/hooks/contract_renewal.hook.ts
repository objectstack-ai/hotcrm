...
const ContractRenewalCheck: Hook = {
  ...
  handler: async (ctx) => {
    ...
    try {
      ...
      const existing = await db.find('opportunity', {
        filters: [
      ...
      const opportunity = await db.insert('opportunity', {
...
const ContractExpirationAlert: Hook = {
  ...
  handler: async (ctx) => {
    ...
    try {
      ...
      await db.update('contract', newDoc._id, {
...
export default [ContractRenewalCheck, ContractExpirationAlert];
