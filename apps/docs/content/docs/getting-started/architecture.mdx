...
```
┌─────────────────────────────────────────────────┐
│          Business Packages Layer                │
│  (CRM, Finance, HR, Marketing, Products, etc.)  │
├─────────────────────────────────────────────────┤
│          AI Service Layer (@hotcrm/ai)          │
│  (Model Registry, Predictions, Caching)         │
├─────────────────────────────────────────────────┤
│      ObjectStack Runtime Engine                 │
│  (ObjectQL, Metadata Registry, Vector Store)    │
├─────────────────────────────────────────────────┤
│          Data & Infrastructure Layer            │
│  (SQLite/PostgreSQL, Redis, S3, etc.)          │
└─────────────────────────────────────────────────┘
```
...
├── account_ai.action.ts        # AI actions for accounts
├── account_enrichment.hook.ts  # Auto-enrichment trigger
├── opportunity.object.ts       # Opportunity schema
├── opportunity.hook.ts         # Stage validation logic
├── lead_conversion.action.ts   # Lead conversion API
└── plugin.ts                   # CRM plugin initialization
```
...
└── my-custom-package/
    ├── src/
    │   ├── custom_object.object.ts
    │   ├── custom_logic.hook.ts
    │   ├── custom_api.action.ts
    │   └── plugin.ts
    ├── package.json
    └── tsconfig.json
```
