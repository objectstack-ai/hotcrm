...
> 进度:计划 8 节中 **7 节已执行**(第 5 节 flows 与第 6 节权限矩阵执行中,见 §10)。
...
- **本轮共立 14 个新 issue**(#680–#684、#688–#693、#696、#698),另给 11 个既有 issue
  回填 rc.2 实测判定(#494 #508 #509 #510 #521 #522 #524 #525 #526 #528 #645 #661 #548 #682)。

**对 GA 的建议**:平台阻断项已基本清空(唯 #508 待修,且它有单条记录的绕行路径)。
CRM/console 侧的硬伤是 **#688**(线索 UI 无法创建,p0)与 **#548**(本轮取得定量证据:
被记录过活动的记录连 admin 都无法删除,见 §8)。在这两条修复、§11 清单跑通前,
不要认为 UI 层已通过验收。flows 与权限矩阵结论待 §10 两个执行中阶段补齐。
...
| **24 条 flows 全量** | ⏳ **执行中**(W3)—— 含 #684 用户会话路径判定 |
| **5 profile 权限矩阵** | ⏳ **待执行**(W4,W3 之后串行) |
