...
export const InvoiceCollectionFlow = {
  ...
  nodes: [
    ...
    },
    {
      ...
      config: {
        rules: [
          { label: 'Escalate', condition: { dialect: 'cel', source: 'days_overdue > 60' } },
          { label: 'Write Off', condition: { dialect: 'cel', source: 'days_overdue > 120' } },
          { label: 'Wait', condition: { dialect: 'cel', source: 'days_overdue <= 60' } },
        ],
      },
      position: { x: 0, y: 300 },
    },
  ...
  edges: [
    ...
    { id: 'e4', source: 'escalate_decision', target: 'escalate_to_manager', type: 'default' as const, condition: { dialect: 'cel', source: 'days_overdue > 60 && days_overdue <= 120' }, label: 'Escalate' },
    { id: 'e5', source: 'escalate_decision', target: 'write_off', type: 'default' as const, condition: { dialect: 'cel', source: 'days_overdue > 120' }, label: 'Write Off' },
    { id: 'e6', source: 'escalate_decision', target: 'end', type: 'default' as const, condition: { dialect: 'cel', source: 'days_overdue <= 60' }, label: 'Wait' },
