...
- **Trust Wall**: Filters PII and toxic content before sending to LLMs.

### 3. Application Layer
The middle tier handles business logic validation and orchestration.

**Components:**
- **ObjectStack Server**: The core runtime.
- **Triggers**: Transactional logic (e.g., "After Update Opportunity").
- **Actions**: Callable functions exposed to the API and AI agents.

### 4. Data Layer
The foundation. It manages structured (relational) and unstructured (vector) data side-by-side.

**Components:**
- **ObjectQL Engine**: Translates high-level queries into optimized database calls.
- **Metadata Registry**: The "DNA" of the system (Objects, Fields, Permissions).
