...
import opportunityObj from '../src/sales/objects/opportunity.object';
import opportunityPage from '../src/sales/pages/opportunity_detail.page';
...
import territoryHook from '../src/sales/objects/opportunity_territory.hook';
...
當你對 LLM 進行 mock 時，技能是確定性的。範例測試的是一個假想的 `renewal-pitch` 技能（內建技能位於 `src/sales/skills/` 與 `src/service/skills/`）：

```ts
// test/renewal-pitch.skill.test.ts
import { runSkill } from '@objectstack/runtime/ai/testing';
import renewalPitch from '../src/sales/skills/renewal-pitch.skill';
...
- ✅ 讓 E2E 測試少而 **關鍵** —— 涵蓋登入、線索轉化、成交、工單解決。
