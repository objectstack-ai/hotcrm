...
- ✅ **不要把自動化掛在轉換上** —— 根本沒有可掛的東西。轉換表是一條驗證規則，在做出這次移動的那一次儲存裡被求值；它寫下一行日誌，不產生任何可供自動化訂閱的事件。只應在某一次移動上跑的自動化，是一條 `record_change` 流程：它在那次儲存上觸發，並在自己的起始條件裡把 `record.stage` 與 `previous.stage` 比對來收窄 **自己** —— `opportunity_won_alert`（`src/flows/opportunity-won-alert.flow.ts`）正是這麼寫的，所以那條祝賀通知只在移入 *Closed Won* 的那一次發出，而不是在這筆已贏商機之後的每一次編輯上都發。
