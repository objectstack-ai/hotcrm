...
**Log a Meeting** is the same shape with a held meeting. **Schedule a Meeting** books one for later and writes the first two only: a booking has not happened yet, so it belongs on the calendar and in the record's **Events** related list rather than on the "what happened" timeline. [Meetings & Calls](/docs/sales/meetings-and-calls) is the full account: what each form asks for, Planned vs Held, what attendee rows hold, and how a held call on a case stamps its First Response Date.
